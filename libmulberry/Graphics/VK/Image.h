...
namespace mulberry::vk
{
    ...
    template <typename T>
    std::vector<T> Image::GetRawData(const ImageAspect &aspect)
    {
        auto srcImg = this;
        ...
        Context::GetDevice()->GetGraphicsCommandPool()->SubmitOnce([&](CommandBuffer *cmd)
                                                                   {
        ...
        T *data = nullptr;
        vkMapMemory(Context::GetInstance().GetDevice(), stagingBuffer->GetMemory(), 0, VK_WHOLE_SIZE, 0, (void **)&data);
