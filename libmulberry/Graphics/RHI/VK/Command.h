...
#define COMMAND_POOL_DECL(name)                                             \
	...
	public:
		...
		virtual void BindDescriptorSets(class PipelineLayout *layout, uint32_t firstSet, const std::vector<const DescriptorSet *> &descriptorSets, const std::vector<uint32_t> &dynamicOffsets = {});
		virtual void BindPipeline(class Pipeline *pipeline) const;

		virtual void CopyImage(class Image *dstImage, class Image *srcImage, const std::vector<VkImageCopy> &copyRegions);

		virtual void CopyBuffer(const class Buffer &dst, const Buffer &src, VkBufferCopy bufferCopy);

		virtual void CopyImageFromBuffer(class Image *dst, class Buffer *src);

		virtual void Reset();

		virtual void Submit(const std::vector<PipelineStage> &waitStages = {}, const std::vector<Semaphore *>& waitSemaphores = {}, const std::vector<Semaphore *>& signalSemaphores = {}, Fence *fence = nullptr) const;
	...
	public:
		...
		void BindVertexBuffers(uint32_t firstBinding, uint32_t bindingCount, const std::vector<class VertexBuffer *> &pBuffers, const std::vector<uint64_t> &pOffsets);
		void BindVertexBuffers(uint32_t firstBinding, uint32_t bindingCount, const std::vector<class VertexBuffer *> &pBuffers);
	...
	public:
		...
		~TransferCommandBuffer() override;
	};
}