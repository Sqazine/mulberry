...
namespace VK
{
        ...
        : m_NeededDeviceExtensions(neededDeviceExtensions)
    {
        ...
        m_QueueIndices = FindQueueFamilies(m_PhysicalDeviceHandle, Context::GetInstance()->GetVKSurfaceKHRHandle());
...
#if _DEBUG
        deviceCreateInfo.enabledLayerCount = Context::GetInstance()->GetNeededValidationLayers().size();
        deviceCreateInfo.ppEnabledLayerNames = Context::GetInstance()->GetNeededValidationLayers().data();
#endif
    ...
    VkPhysicalDevice Device::SelectPhysicalDevice()
    {
        ...
        vkEnumeratePhysicalDevices(Context::GetInstance()->GetVKInstanceHandle(), &physicalDeviceCount, nullptr);
        if (physicalDeviceCount == 0)
            std::cout << "No GPU support vulkan" << std::endl;
        std::vector<VkPhysicalDevice> physicalDevices(physicalDeviceCount);
        vkEnumeratePhysicalDevices(Context::GetInstance()->GetVKInstanceHandle(), &physicalDeviceCount, physicalDevices.data());
        ...
        for (auto phyDev : physicalDevices)
        {
            ...
            if (extSatisfied && FindQueueFamilies(phyDev, Context::GetInstance()->GetVKSurfaceKHRHandle()).IsComplete())
                result = phyDev;
