...
namespace Loader
{
    ...
    ImgData LoadImgDataFromFile(const std::string_view path, bool isFlip)
    {
        ...
        uint8_t *data = stbi_load(path.data(), (int32_t *)&tmp.width, (int32_t *)&tmp.height, (int32_t *)&tmp.channels, STBI_default);
        tmp.pixels = std::vector<uint8_t>(data, data + (tmp.width * tmp.height * tmp.channels));
        return tmp;
    }
}
